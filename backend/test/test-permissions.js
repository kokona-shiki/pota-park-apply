...
const testPermissions = async () => {
  ...
  console.log('1️⃣ 检查 system_admin 的权限:');
  ...
  if (hasReviewPermissions) {
    console.log('  ❌ 失败: system_admin 仍然拥有审核权限');
    console.log('  审核权限:', systemAdminPerms.rows.filter((p) => reviewPermissions.includes(p.permission_code)));
    return false;
  } else {
    console.log('  ✅ 通过: system_admin 不再拥有审核权限');
  }
  ...
  if (!reviewerHasReviewPerms) {
    console.log('  ❌ 失败: park_reviewer 没有必要的审核权限');
    return false;
  } else {
    console.log('  ✅ 通过: park_reviewer 拥有审核权限');
  }
  ...
  if (!repHasAllReviewPerms) {
    console.log('  ❌ 失败: pota_representative 没有完整的审核权限');
    return false;
  } else {
    console.log('  ✅ 通过: pota_representative 拥有完整的审核权限');
  }
  ...
  rolePermissions.rows.forEach((row) => {
    console.log(`  ${row.role}: ${row.permission_count} 个权限`);
  });

  console.log('\n✅ 所有测试通过!');
  console.log('\n📝 权限调整总结:');
  console.log('  - system_admin: 只保留用户管理权限,不再参与公园审核');
  console.log('  - park_reviewer: 拥有公园申请审核权限');
  console.log('  - pota_representative: 拥有公园申请审核 + POTA 系统录入权限');
