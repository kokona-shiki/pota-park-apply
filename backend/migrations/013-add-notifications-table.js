...
export const down = async () => {
  ...
  await query(`DELETE FROM permissions WHERE permission_code IN (
    'create_global_notification',
    'edit_global_notification',
    'publish_global_notification',
    'withdraw_global_notification',
  ...
  await query(`DROP INDEX IF EXISTS idx_notification_drafts_created_by`);
  await query(`DROP TABLE IF EXISTS notification_drafts`);

  await query(`DROP INDEX IF EXISTS idx_notifications_notification_mode`);
  await query(`DROP INDEX IF EXISTS idx_notifications_status`);
  await query(`DROP INDEX IF EXISTS idx_notifications_is_global`);
